...
double z_coord_min = 0.0;
double z_coord_max = 0.0;
...
void pclCallback(const sensor_msgs::PointCloud2 msg){
  ...
  for (int i = 0; i < (*pcl_cloud_nan).size(); i++) {
    if (!std::isnan(pcl_cloud_nan->points[i].x) && !std::isnan(pcl_cloud_nan->points[i].y) && !std::isnan(pcl_cloud_nan->points[i].z)) {
      ...
      if ((source_point.point.x > x_coord_min) && (source_point.point.x < x_coord_max) && (source_point.point.y > y_coord_min) && (source_point.point.y < y_coord_max) && (source_point.point.z > z_coord_min) && (source_point.point.z < z_coord_max)) {
        pcl_cloud_size ++;
        sum_x += source_point.point.x;
        sum_y += source_point.point.y;
        sum_z += source_point.point.z;
...
int main(int argc, char** argv){
  ...
  pnh.getParam("z_coord_min", z_coord_min);
  pnh.getParam("z_coord_max", z_coord_max);
